...
function showfooter()
{
    ...
    if ($loadonly_content == "loadonly_content" && $pageid == $getpage && !is_user_logged_in()) {
        echo "<style>
        .site-header,#site-header,.sidebar-main, #header,header,#footer,footer,
        .sidebar,#sidebar  
        .site-sidebar,#site-sidebar ,.site-footer,#site-footer{
        display:none !important;
        }
        .content-area,#main,#primary{
            width:100% !important;
...
function Register_Csm_settings()
{
    ...
    register_setting('csm-settings', 'dis_more_option');
...
function Csm_settings()
{
    ...
    ?>
    <div class="wrap"><h2><?php _e('Coming Soon Mode', 'obwoos'); ?>
</h2>
</div>
    <div class="wrap">
        <div class="postbox">
                    ...
                    <table class="form-table">
                        <tr valign="top">
                            ...
                            <td>
                                ...
                                ?> 
                                <label><input type="radio" name="csm_mode" value="live" 
                                <?php echo $csm_mode == 'live' ? 'checked' : ''; ?> /> 
                                <?php _e('Live', 'csm')?>
                                <p class="description">If Live is selected, then a website is visible for all.</p>
                                </label><br />
                               <!-- <label><input type="radio" name="csm_mode" value="maintainance" 
                               <?php echo $csm_mode == 'maintainance' ? 'checked' : ''; ?> /> 
                               <?php _e('Maintainance', 'csm')?></label><br />-->
                               <label><input type="radio" name="csm_mode" value="comming-soon" <?php echo $csm_mode == 'comming-soon' ? 'checked' : ''; ?> /> <?php _e('Coming Soon', 'csm')?>
                               <p class="description">If Coming Soon is selected, then site visitors will redirect to the dedicated page.</p>
                            </label><br />
                        </td>
                    </tr>
            ...
            <th scope="row"><?php _e('Live Site Access', 'csm'); ?>:
        </th>
        <td>
            <?php
            $csm_who_can_access = get_option('csm_who_can_access', 'logged');
            ?>
            <label><input type="radio" name="csm_who_can_access" value="logged" 
            <?php echo $csm_who_can_access == 'logged' ? 'checked' : ''; ?> /> 
            <?php _e('All Logged-in Users', 'csm')?>
        </label><br /><p></p>
        <label><input type="radio" name="csm_who_can_access" value="custom" 
        <?php echo $csm_who_can_access == 'custom' ? 'checked' : ''; ?> />
        <?php _e('Selected User Roles', 'csm')?>
    </label><br />
    ...
    <label><input type="radio" class="dis_more_option" name="csm_appearance" value="loadonly_content" 
    <?php echo $loadonly_content == 'loadonly_content' ? 'checked' : ''; ?> /> 
    <?php _e('Display Page Content Only', 'csm')?>
    </label><br /><p></p>
    <label><input type="radio" class="dis_more_option" name="csm_appearance" value="dis_more_option" 
    <?php echo $loadonly_content == 'dis_more_option' ? 'checked' : ''; ?> /> 
